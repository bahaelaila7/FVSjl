#!/usr/bin/env bash
# ne_oracle.sh — produce live FVS Northeast ground-truth output for a .key file.
#
# The NE port's oracle. The shipped bin/FVSne fails on this box's GLIBC (needs
# 2.38/2.43), so — exactly like the SN harness (fortran_baseline.sh) — we relink
# a working binary (/workspace/FVSjl/tmp/oracles/FVSne_new) from the resolved .o files in the NE build dir
# plus the glibc shim, then run it on a keyfile in an isolated dir.
#
# Usage:  ne_oracle.sh <keyfile> <outdir>      → prints <outdir>, writes <stem>.sum etc.
#
# NOTE: tests/FVSne/net01.sum.save is STALE (built from source dated 20250930; the
...
BUILDDIR="${FVSNE_BUILDDIR:-/workspace/ForestVegetationSimulator/bin/FVSne_buildDir}"
BIN="${FVSNE_BIN:-/workspace/FVSjl/tmp/oracles/FVSne_new}"
