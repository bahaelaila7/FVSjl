...
@testset "FVSjl" begin
    ...
    include("unit/test_wwpb_bmatct_multi.jl") # WWPB mode-2 multi-stand landscape dispersal (bmatct.f/bmdrv.f MXSTND>1): bmatct_multi! SCORE/PROP interstand redistribution + Outside-World BKPIN/BKPOUT bit-exact vs gfortran-16 driver golden (3 OW regimes); bmdrv_multi! collapses to single-stand at MXSTND=1
    ...
    include("unit/test_ppe_landscape_live.jl") # PPE mode-2 LIVE interstand-beetle coupling (ppe_run_landscape_live!): the master-cycle lockstep barrier (cooperative Task+Channel rendezvous at the WWPB seam) runs ONE bmdrv_multi! dispersal across placed neighbors in-flight. EQUIVALENCE: standalone-decisions == in-flight kills bit-exact + independent replay of the fixed decisions == LIVE .sum byte-identical (plumbing adds zero divergence); COLLAPSE: MXSTND=1 == single-stand run_keyfile DISPERSE byte-identical; USE-CASE: outbreak seeded in one stand disperses to its adjacent neighbor (mortality > isolation, monotone with seed) while a far stand is unaffected
    include("integration/test_ppe_harvest.jl") # PPE MXHRVP multistand harvest scheduling — HVSEL (hvsel.f) greedy priority-ranked target-constrained stand selection bit-exact vs gfortran-16 driver-golden over the historical FVSppe objects (8 branch cases + 200-case fuzz); + HVCCUT, evmon PPE policy-vars, LBMEMR/LBUNIN label-set; additive/inert until the master-cycle seam
    include("integration/test_ppe_harvest_e2e.jl") # PPE MXHRVP END-TO-END: ppe_run_landscape_harvest! (hvaloc.f coordinator) vs the proven FVSppe oracle golden (msp landscape) — SELECT outcomes + cyc0 numerics BIT-EXACT (BBA 77.39206==0.7739E+02, SELECTED 232.18, 23.2%); cyc1+ CREDIT residual cornered to the EC-variant before-thin-BA growth straddle (inherited by CREDIT=BBA, not a MXHRVP defect)
    include("integration/test_ppe_harvest_diff.jl") # PPE MXHRVP DIFFERENTIAL end-to-end (msp_diff.key CREDIT=SELECTED*BBA, TARGET=150): HVYLDS(sel)=BBA != HVTHIN(not)=0 drives a SUBSET selection (1 full + 1 partial status-4/HVPART + 1 not) filling to target — vs the FVSppe oracle. cyc0 HVPART=0.938 BIT-EXACT (==93.8%); cyc1+ HVPART cornered to the EC growth straddle; the full/partial/not assignment cornered to the RDPSRT unstable-sort tie-break on equal priorities (#206-class)
    include("integration/test_ppe_harvest_composite.jl") # PPE MXHRVP chunk-F composite-MATERIALIZATION (the harvest EFFECT): the AGPLABEL-gated THINBTA fires on selected stands, the post-harvest COMPOSITE (_ppe_aggregate) reflects the removals vs the FVSppe golden msp_thin — post-thin TPA BIT-EXACT every cycle (536→36→35→35, harvest genuinely materialized), cyc0 volumes ~bit-exact, cyc1+ volumes cornered to the EC post-thin-regrowth growth straddle (#207). [THINBTA verdict: the earlier "over-thin" was a keyfile column-misalignment artifact; FVSjl THINBTA is correct]
    include("integration/test_ppe_harvest_lhvmxc.jl") # PPE MXHRVP LHVMXC max-contiguous-clearcut: SPCNTG contiguous-acreage bit-exact vs gfortran-16 driver-golden (spcntg.f + stubbed SPLAAR/SPNBBD) + hvsel! contiguous-acre veto
    include("integration/test_ppe_harvest_ihvext.jl") # PPE MXHRVP IHVEXT=1 external-selection rule (hvsel.f:167-175: skip priority<=0, EXACT off) bit-exact vs gfortran-16 driver-golden
    include("integration/test_ppe_harvest_ihvext_bridge.jl") # PPE MXHRVP IHVEXT external-I/O bridge: PPE_FFERdAccess.txt read-back (ppe_read_rdaccess) bit-exact vs sprdrd.f/sprdis.f driver-golden + coordinator priority-override
