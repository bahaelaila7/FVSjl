    ...
    if !isfile(key)
        ...
        try
            ...
            for l in split(sumtxt, "\n")
                ...
                # float-sum vs RENDERED-INTEGER: jl's Σ rounds to the .sum integer (rendered-==; measured max
                # 0.44/0.465 < 0.5 half-width ⇒ rounds identically). Was ≤0.5 tolerance.
                @test round(Int, agg[y][1]) == parse(Int, f[3])       # Σ TPA renders to the stand-TPA integer
                @test round(Int, agg[y][2]) == parse(Int, f[9])       # Σ(TCuFt·TPA) renders to the total-cuft integer
            end
        finally
