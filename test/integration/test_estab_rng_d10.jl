...
@testset "establishment :estab RNG fidelity (D10) vs live FVSsn" begin
    ...
    if !isfile(key)
        ...
        # (3) mean DBH at a LATE cycle (2042, ~9 growth cycles). Live 9.8062. This was long EXPOSED as a broken
        # "grown-Float32 accumulation floor" (jl 9.812015, Δ0.0058) — but that Δ was NOT irreducible: it was the
        # stand_pct! percentile TIE-BREAK bug (standstats.jl used a stable sortperm where FVS's dense.f/PCTILE
        # indexes trees via the UNSTABLE RDPSRT quicksort, gradd.f:186). On tie-heavy stands the wrong tie-order
        # mis-assigns the VARMRT/DGF competition percentile, perturbing which trees die/grow; fixing it to _rdpsrt!
        # collapses Δ0.0058 → <5e-5 (a ~100× reduction, robustly within the 4-dec half-width). Now bit-tracking live.
        lp42 = snap[2042].lp
        @test isapprox(sum(lp42) / length(lp42), 9.8062f0; atol = 5f-5)
    end
end
