...
@testset "FERTILIZE growth response vs Fortran" begin
    ...
    if !isfile(key) || !isfile(sav)
        ...
        if length(jl) == length(ft)
            ...
            @test all(_fzcol(jl[i], 9)  == _fzcol(ft[i], 9)  for i in 1:length(jl))  # TCuFt — bit-exact (cftopk pre-growth bark, FVS vols.f:150)
            @test_broken all(_fzcol(jl[i], 10) == _fzcol(ft[i], 10) for i in 1:length(jl))  # MCuFt — bit-exact (cftopk pre-growth bark, FVS vols.f:150)
            ...
            @test_broken all(_fzcol(jl[i], 12) == _fzcol(ft[i], 12) for i in 1:length(jl))  # BdFt — bit-exact (cftopk pre-growth bark, FVS vols.f:150)
        end
    end
end
