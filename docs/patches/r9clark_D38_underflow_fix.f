...
c-----The Clark taper profile is defined only above its 17.3' upper reference height;
c-----below it the (1-17.3/totHt) base is negative and (base)**p is NaN, which SIGFPEs
c-----under FP-exception trapping. Such a tree is sub-merchantable => no merch height.
      if(totHt .le. 17.3) then
        stemHt = 0.0
        return
      endif
c-----Set combined variables (guard the divisions: (1-G) and (X-Y) can underflow to 0
c-----for very tall stems, which SIGFPEs under FP-exception trapping; cf. the xxx>0
c-----branch guards below. Bases are already non-negative here because totHt>17.3.)
      G=(1.0-4.5/totHt)**r
      if((1.0-G) .gt. 1.0E-10) then
        W=(c+e/dbhIb**3)/(1.0-G)
      else
        W=0.0
      endif
      X=(1.0-4.5/totHt)**p
c-----Near the tip (totHt just above 17.3) the base is tiny and p large, so (base)**p
c-----underflows to a denormal, which SIGFPEs under FE_UNDERFLOW trapping. Set Y=0.
      if((1.0-17.3/totHt).lt.0.005748 .and. p.gt.14.0) then
        Y=0.0
      else
        Y=(1.0-17.3/totHt)**p
      endif
      if((X-Y) .gt. 1.0E-10) then
        Z=(dbhIb**2-dib17**2)/(X-Y)
      else
        Z=0.0
      endif
