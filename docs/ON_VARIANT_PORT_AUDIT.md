...
## ✅ ON CORE COMPLETE — growth + mortality + VOLUME end-to-end, full cyc0 `.sum` all columns bit-exact.

## Remaining (all NON-growth refinements / classification, dep-ordered)

1. **FORTYP / size / stock classification** (`fvsstd.f`/`revise.f`) — the only remaining non-volume cyc0
   `.sum` delta (999/55 vs oracle 125/11). Classification, not growth/vol.
2. **CCF** — ON open-grown crown-**width** `canada/on/cwcalc.f` (IWHO=1) + CCFCAL + a `stand_ccf` Ontario
   branch (distinct from the crown-**ratio** TWIGS model). cyc0 CCF renders `15` vs oracle `****` overflow
   on the degenerate metric-BAF stand. Also the metric `.sum` cyc1 IOSUM(3) TPA-column artifact (oracle
   prints 24 vs BA-implied ~950; a metric sumout.f quirk, not a jl bug).
