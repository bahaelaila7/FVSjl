...
| VOLEQNUM | per-species cubic volume-equation override (VEQNNC, initre.f:5061) | 🟡 **DONE for cubic** (kw_voleqnum! → Control.voleqnum_overrides → apply_voleqnum_overrides! after VOLEQDEF; species via resolve_species alpha/FIA or −N group). Bit-exact vs Fortran on total/merch/sawtimber cubic (test_voleqnum.jl, SM→AB eq: total cuft 1368→1419). Board feet (col 12) follows the cubic eq in FVSjl but Fortran keeps VEQNNB at the default → ~5% divergence (the shared board-foot separate-equation limitation, DIVERGENCES.md) |
