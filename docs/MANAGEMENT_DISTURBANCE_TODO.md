...
| TCONDMLT | tree-condition cut-priority weight (TCWT·IMC, cuts.f:1074/1424) | ✅ **DONE** (kw_thin! icflag 202 → cuts! PASS 1 sets Control.total_wt; the RDPSRT cut key is now the full cuts.f:1074 formula via `_cut_pref_wt` = IORDER + TCWT·IMC + SPCLWT·ISPECL, IMC=clamp(mort_code,1,3); + basdam code-55 → t.special/ISPECL). Bit-exact vs Fortran (test_tcondmlt.jl: IMC=3 maples cut first ≠ no-TCONDMLT). Default TCWT=SPCLWT=0 ⇒ key unchanged |
