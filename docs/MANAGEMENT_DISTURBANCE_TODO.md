...
| YARDLOSS | yarding-loss → scales removed merch/saw/bdft by (1−prlost) **and feeds the FFE down-wood/snag/crown fuel pools** | ✅ **DONE (.sum volume scaling)** — was a SILENT gap (active in snt01.key/sn.key, unrecognized → ignored; found by the unrecognized-keyword guard). kw dispatch sets `control.yardloss_prlost`; `cuts!` scales the returned removed merch/saw/board by (1−PRLOST), total cubic + TPA unchanged (cuts.f:1387). Verified on cut_specpref (merch 272→136 @ PRLOST=0.5, cuft/tpa unchanged; test_cuts_coverage.jl). Inert on snt01/sn (their thins remove only sub-merch trees) — which is why the silent gap hid. 🧊 **fuel-pool routing (DSNG/SSNG/CTCRWN) still C7** (matters only with active FFE + a merch-removing thin). |
...
| `cut_yardloss` | YARDLOSS | the THINDBH here removes 0 merch → .sum-neutral; the volume-scaling semantic is ✅ ported and tested on cut_specpref (merch ×(1−PRLOST)). Fuel-pool routing still C7. |
...
**D. Out of scope here (C7/C8):** YARDLOSS *fuel-pool routing* (C7; the .sum volume scaling is ✅ done), FFE fire, insects, root disease
