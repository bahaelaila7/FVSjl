...
| DEFECT (per-tree) | per-tree CF/BF/MC defect packed in the tree DEFECT field (vols.f:294 ICDF=DEFECT/1e6, :352 IBDF) | ⛔ (FVSjl parses no DEFECT column into the tree record yet — needs .tre column layout + digit-unpacking; ICDF=max(per-tree, CFDEFT)). CFLA0/CFLA1/BFLA0/BFLA1 log-linear form model is **verified no-op for SN** (only ever set 0/1 in grinit/initre — not a gap) |
