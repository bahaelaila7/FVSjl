...
| MCDEFECT / BFDEFECT | per-species cubic / board-foot defect curves (CFDEFT/BFDEFT, sdefet.f + FVSsn vols.f) | 🟡 `kw_mcdefect!`/`kw_bfdefect!` → `Control.sp_cf_defect`/`sp_bf_defect`; cubic cuts pulpwood by ICDF%, board cuts BFV+SCFV by IBDF%, then MCFV=PULPV+post-board SCFV — all via ALGSLP over DBHCLS in `compute_volumes!`; bit-exact vs Fortran incl. the coupled case (`test_mcdefect.jl`). Per-tree DEFECT input deferred; CFLA form model verified no-op for SN (DIVERGENCES.md) |
