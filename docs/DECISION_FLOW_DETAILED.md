...
| VOLEQNUM | per-species cubic NVEL volume-equation override (initre.f:5061) | 🟡 `kw_voleqnum!` → `apply_voleqnum_overrides!` (after VOLEQDEF; alpha/FIA/−group species). Cubic columns bit-exact vs Fortran (`test_voleqnum.jl`); board feet follows the cubic eq (shared board-foot limitation, DIVERGENCES.md) |
| VOLUME / BFVOLUME | per-species cubic / board-foot merch-standard override (volkey.f) | ✅ `kw_volume!`/`kw_bfvolume!` → `apply_volume_overrides!`. DBHMIN gate (`test_volume_override.jl`) + BFVOLUME bit-exact via BFPFLG=0 separate board call + Region-8 zeroing (`test_bfvolume.jl`) |
