...
function Control()
    ...
    Control(
        ...
        false, false, false, false, false, false, false, false, false, false, nothing, # dbs_cutlist, dbs_climate, dbs_canprofile, dbs_strclass, dbs_calibstats, dbs_mistoe, mistprt_on, dbs_bm_main, dbs_bm_tree, dbs_bm_vol, cutlist_capture
...
# Forward declaration for the Western Root Disease (WRD) state; concrete
# `RootDiseaseState` is in engine/root_disease.jl (included after this file).
...
# Forward declaration for the Douglas-fir Tussock Moth (DFTM) defoliator-model
# state; concrete `DftmState` is in engine/dftm.jl (included after this file).
# `nothing` until a DFTM keyword activates it — inert for every non-DFTM run (no
# engine seam wired yet).
abstract type AbstractDftmState end

# Forward declaration for the White Pine Blister Rust (WPBR) canker-model state;
# concrete `WpbrState` is in engine/wpbr.jl (included after this file). `nothing`
# until a BRUST keyword activates it — inert for every non-WPBR run (no engine
# seam wired yet).
