...
# Per-variant mistletoe table dispatch (misint{v}.f DATA differs by species order). Only UT
# is ported to its own table so far; the rest keep the IE table (correct where DF/LP/PP align;
# their variant-specific woodland hosts are a documented follow-up). Returns (FIT, DGP, PMC, MAXSP).
@inline function _mis_tables(v)
    v isa Utah && return (UT_MIS_FIT, UT_MIS_DGP, UT_MIS_PMC, 24)
    return (IE_MIS_FIT, IE_MIS_DGP, IE_MIS_PMC, 23)
end
