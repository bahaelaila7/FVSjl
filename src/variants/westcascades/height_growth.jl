# =============================================================================
# height_growth.jl (westcascades) — WC large-tree height growth (wc/htgf.f + findag.f
# + htcalc.f). Chunk 4.
#   wc_htcalc(sindx,ispc,ag)        — site-index height-at-age curve (wc/htcalc.f; per-species
#                                     branches: SF Hoyer, WF/GF Cochran, AF/ES Alexander RM-32,
#                                     RF Dolph, NF Herman, IC/JP/PP Barrett, LP Dahms, SP/WP
#                                     Curtis, WH Wiley, MH Means, RA Harrington, LL Cochran,
#                                     WO King, "misc" Curtis for the rest; blank ⇒ 0).
#   wc_findag(ispc,d,d2,h,sindx)    — invert the site curve for (SITAGE,SITHT); HTMAX/HTMAX2 are
#                                     HT/DBH-ratio caps HDRAT1(MAPHD)·D(+D2)+HDRAT2 (findag.f).
#   height_growth!(s,::WestCascades;scale) — per-tree HTG: RW (ISPC17) LTHTG special, OWO
#                                     (ISPC28) King HT-DBH patch, else FINDAG→10-yr POTHTG·HTGMOD
#                                     (HGMDCR crown + HGMDRH gen. Chapman-Richards, WTCR=.25),
#                                     size caps, SCALE·XHT·exp(HTCON)·MISHGF.
#
# The DEFAULT branch potential is a 10-YEAR site-curve rise (AGP10 = SITAGE+10, wc/htgf.f:283),
# so htg_period(WestCascades)=10 and the engine passes scale = FINT/10 (SCALE=FINT/YR, htgf.f:141).
# MEASURED vs FVSwc_clean wct01 cycle-1 HTGF DEBUG: SCALE=1.0 at the 10-yr cycle, HTCON≡0 (no
# height calibration), XHT≡1 (no HTGMULT).  See docs/WC_VARIANT_PORT_AUDIT.md (chunk-4 verdict).
# =============================================================================

# wc/htgf.f DATA — relative-height (RH) modifier coeffs by species shade tolerance (39 species).
const WC_HT_RHR = Float32[
    16.0,16.0,16.0,20.0,16.0, 16.0,15.0,16.0,20.0,16.0, 12.0,13.0,15.0,15.0,13.0,
    15.0,20.0,20.0,20.0,20.0, 20.0,13.0,13.0,13.0,15.0, 12.0,12.0,15.0,13.0,12.0,
    15.0,12.0,20.0,20.0,12.0, 13.0,12.0,15.0,15.0]
const WC_HT_RHYXS = Float32[
    0.15,0.15,0.15,0.20,0.15, 0.15,0.10,0.15,0.20,0.15, 0.01,0.05,0.10,0.15,0.05,
    0.10,0.20,0.20,0.20,0.20, 0.20,0.05,0.05,0.05,0.10, 0.01,0.01,0.10,0.05,0.01,
    0.10,0.01,0.20,0.20,0.01, 0.05,0.01,0.10,0.10]
const WC_HT_RHM = fill(1.10f0, 39)             # wc/htgf.f DATA RHM / MAXSP*1.10 /
const WC_HT_RHB = Float32[
    -1.20,-1.20,-1.20,-1.10,-1.20, -1.20,-1.45,-1.20,-1.10,-1.20, -1.60,-1.60,-1.45,-1.45,-1.60,
    -1.45,-1.10,-1.10,-1.10,-1.10, -1.10,-1.60,-1.60,-1.60,-1.45, -1.60,-1.60,-1.45,-1.60,-1.60,
     0.10,-1.60,-1.10,-1.10,-1.60, -1.60,-1.60,-1.45,-1.45]      # note sp31 (WB) = +0.10 (per DATA)
const WC_HT_CRA = 100.0f0; const WC_HT_CRB = 3.0f0; const WC_HT_CRC = -5.0f0
const WC_HT_RHK = 1.0f0;   const WC_HT_RHXS = 0.0f0

# wc/findag.f DATA — MAPHD (39→8 HT/DBH-ratio classes) + HDRAT1/HDRAT2, AGMAX=200 all species.
const WC_HT_MAPHD = Int[
    1,1,1,2,2, 6,2,3,3,2, 4,5,5,5,5, 6,6,3,7,7, 8,8,8,8,8, 8,8,8,3,3, 3,3,3,8,8, 8,8,6,6]
const WC_HT_HDRAT1 = Float32[4.3396271,4.3149844,3.2412923,2.3475244,5.5324838,6.3657425,4.0156013,3.9033821]
const WC_HT_HDRAT2 = Float32[43.9957174,39.6317079,62.7139427,65.7622908,18.6043842,16.2223589,51.9732476,59.3370816]
const WC_HT_AGMAX = 200.0f0

"""
    wc_htcalc(sindx, ispc, ag) -> hguess

wc/htcalc.f — potential height at age `ag` for species `ispc` on site `sindx`. Blank/future
species (6, 38) and any unlisted case return 0.
"""
@inline function wc_htcalc(sindx::Float32, ispc::Int, ag::Float32)
    ...
    elseif ispc == 28                                        # WO — King (Weyerhaeuser #8)
        z = 2500f0/(sindx-4.5f0)
        return ag*ag/(-0.954038f0 + 0.109757f0*z + (5.58178f-2+7.92236f-3*z)*ag +
               (-7.33819f-4+1.97693f-4*z)*(ag*ag)) + 4.5f0
    elseif ispc == 8 || (16 <= ispc <= 18) || ispc == 21 || (23 <= ispc <= 27) ||
           ispc == 29 || (31 <= ispc <= 37) || ispc == 39                # "misc" — Curtis
        s45 = sindx - 4.5f0
        return s45 / (0.6192f0 - 5.3394f0/s45 + 240.29f0*ag^(-1.4f0) +
               (3368.9f0/s45)*ag^(-1.4f0)) + 4.5f0
    else                                                     # blank/future (6, 38)
        return 0.0f0
    end
end

"""
    wc_findag(ispc, d, d2, h, sindx) -> (sitage, sitht, agmax, htmax, htmax2)

wc/findag.f — solve site age/height from tree height `h` by stepping AG by 2 up the site curve
(TOLER=2, INCRNG flatten-detection). HTMAX/HTMAX2 = HDRAT1(MAPHD)·D(+D2)+HDRAT2 (HT/DBH ratio
caps). If H≥HTMAX, age is dubbed at 0.10 ft/yr above AGMAX.
"""
@inline function wc_findag(ispc::Int, d::Float32, d2::Float32, h::Float32, sindx::Float32)
    agmax = WC_HT_AGMAX
    mp = WC_HT_MAPHD[ispc]
    ...
    while true
        oldhg = hguess
        incrng = false
        hguess = wc_htcalc(sindx, ispc, ag)
        if hguess >= 1.0f0
            diff = abs(hguess - h)
...
"""
    wc_htg_default(ispc, sindx, d, h, icr_pct, avh, ba, dg, brat) -> htg_preScale

The wc/htgf.f DEFAULT + OWO(28) height increment for one tree, BEFORE SCALE·XHT·exp(HTCON)·MISHGF.
Returns HTG(I) as computed through the size-cap-at-HTMAX2 step (>=0.1). Assumes PROB>0 and
H<=HTMAX (the H>HTMAX and RW branches are handled in height_growth!).
"""
@inline function wc_htg_default(ispc::Int, sindx::Float32, d::Float32, h::Float32,
                                icr_pct::Float32, avh::Float32, ba::Float32, dg::Float32,
                                d2::Float32, sitage::Float32, sitht::Float32,
                                agmax::Float32, htmax2::Float32)
    # potential 10-yr height growth
    local pothtg::Float32
    if sitage >= agmax
        pothtg = (ispc == 2 || ispc == 3) ? (0.2f0 + 0.00264f0*sindx)*10f0 : 0.10f0
    else
        hguess = wc_htcalc(sindx, ispc, sitage + 10.0f0)
        pothtg = hguess - sitht
        if ispc == 28                                        # OWO King HT-DBH patch (wc/htgf.f:300)
    ...
    # modifiers
    cr = icr_pct/100f0
    hgmdcr = WC_HT_CRA * cr^WC_HT_CRB * exp(WC_HT_CRC*cr)
...
"WC height_growth! — wc/htgf.f per-tree periodic height increment into trees.ht_growth."
function height_growth!(s::StandState, ::WestCascades; scale::Float32 = 1.0f0)
    p, t, c = s.plot, s.trees, s.calib
    sd = s.coef.species
    avh = p.avg_height; ba = p.basal_area; slope = p.slope; asp = p.aspect
    @inbounds for i in 1:t.n
        ...
        htcon = c.htg_cor[ispc]                              # HTCON (0 baseline; +ln(HCOR2) if calibrated)
        et = Int(sd[:bark_imap][ispc]); brat = wc_bratio(sd[:bark1][ispc], sd[:bark2][ispc], et, d)
        d2 = d + dg/brat
        local htg::Float32
        if ispc == 17                                        # REDWOOD — LTHTG special (wc/htgf.f CASE 17)
            ...
            htg = lthtg*hgbnd
            htg < 0.1f0 && (htg = 0.1f0)
            htg = scale*htg*exp(htcon)                       # XHT=1
        else                                                 # DEFAULT (+ OWO 28)
            sitage, sitht, agmax, htmax, htmax2 = wc_findag(ispc, d, d2, h, sindx)
            if h > htmax                                     # start-of-cycle HT/DBH cap breach (wc/htgf.f:263)
                if h >= htmax2
                    htg = 0.5f0*dg; htg < 0.1f0 && (htg = 0.1f0)
                    htg = scale*htg*exp(htcon)
                else
                    htg = 0.0f0                              # GO TO 161 with HTG=0
                end
            else
                htg = wc_htg_default(ispc, sindx, d, h, icr_pct, avh, ba, dg, d2,
                                     sitage, sitht, agmax, htmax2)
                htg = scale*htg*exp(htcon)
            end
        end
        # 161: MISHGF (mistletoe, 1 baseline) then SIZCAP(ISPC,4) height cap.
