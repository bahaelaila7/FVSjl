...
function kw_stdinfo!(s::StandState, rec::KeywordRecord)
    ...
    if rec.present[2]
        ...
        elseif s.variant isa EastCascades
            # EC STDINFO field 2 is a habitat/plant-association code that may be ALPHA (e.g. "CWS222")
            # or a numeric sequence number. ec/habtyp.f→hbdecd.f decodes it into the KODTYP index that
...
function kw_database!(s::StandState, rec::KeywordRecord, kr::KeywordReader)
    ...
    while true
        ...
        elseif k == "PPBMTREE"
            s.control.dbs_bm_tree = true   # dbsin.f opt 20: IBMTREE ⇒ FVS_BM_Tree per-size-class detail
        elseif k == "PPBMVOL"
...
function process_keywords!(s::StandState, kr::KeywordReader, base_path::AbstractString)
    ...
    while true
        ...
        elseif kw == "MPB";      kw_mpbin!(s, rec, kr)      # Mountain Pine Beetle block (keywds.f 'MPB'; MPBSTART/POPDYN/INITMORT/QVALUES/… → s.mpb + gated Cole rate-of-loss mortality seam)
