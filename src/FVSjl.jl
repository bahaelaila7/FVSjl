...
include("variants/britishcolumbia/crown_width.jl")           # BC FVS_TreeList crown width (national cwcalc.f BCMAP dispatch)
...
include("variants/inlandempire/crown_width.jl")      # IE FVS_TreeList crown width (national cwcalc.f IEMAP dispatch)
...
include("variants/oregoncoast/crown.jl")             # OC FVS-native rank-Weibull crown CHANGE for IORG=0 trees (oc/crown.f CASE DEFAULT)
include("variants/oregoncoast/ffe_coefficients.jl")  # OC FFE crown-biomass coeffs (fmvinit.f V2T / fmcrow.f ISPMAP / dbh_min)
...
include("variants/southeastalaska/crown_width.jl") # AK FVS_TreeList crown width (national cwcalc.f AKMAP dispatch)
...
include("engine/lpmpb_lpopdy.jl")         # LPMPB LPOPDY population-dynamics path (GARBEL/SURFCE/MPBMOD epidemic) — deterministic, dump-replay validated
