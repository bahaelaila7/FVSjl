...
# dense UT stands most likely to exceed BAMAX (high TPA) — the regime the cap governs
q="""SELECT s.STAND_CN FROM FVS_STANDINIT_COND s
     JOIN (SELECT STAND_CN, SUM(TREE_COUNT) tc FROM FVS_TREEINIT_COND GROUP BY STAND_CN) t
       ON s.STAND_CN=t.STAND_CN
     WHERE s.VARIANT='UT' ORDER BY t.tc DESC LIMIT 20"""
stands=[r.STAND_CN for r in DBInterface.execute(DB,q)]
println("UT dense stands: ", length(stands))
...
println("UT: n=$n bit-exact=$ne cornered(≤3%)=$corn diverged(>3%)=$div jl-crash=$crash no-live=$nolive")
