...
C=== DFTM DUMP-IN pre-DFTM per-record entry state (record order) ===
      DO 9091 I=1,ITRN
        WRITE(0,'(A,I5,3I4,I8,7(1X,Z8.8))') 'DBGIN_TRE ',
     &     I,IMC(I),ICR(I),NORMHT(I),ITRUNC(I),
     &     TRANSFER(WK2(I),1),TRANSFER(HT(I),1),
     &     TRANSFER(HTG(I),1),TRANSFER(DBH(I),1),
     &     TRANSFER(DG(I),1),TRANSFER(PCT(I),1),
     &     TRANSFER(PROB(I),1)
 9091 CONTINUE
...
C     
          CALL TMRANN(PRTOPK) 
      WRITE(0,'(A,I6,1X,Z8.8)') 'DBGTK_PR ',IP,
     &   TRANSFER(PRTOPK,1)
