...
set -u
SRC=/workspace/ForestVegetationSimulator/bin/FVStt_buildDir
BWE=/workspace/ForestVegetationSimulator/wsbwe
WORK=/workspace/.ttwork/wsbwe
OBJ=$WORK/emobj
SHIM=/workspace/.crwork/isoc23_shim.o
OVR="${1:-}"
OUT="${2:-$WORK/FVStt_wsbwe}"
FC=gfortran-16
...
for f in *.f; do
  b=$(basename "$f" .f)
  case "$b" in
    bwebk|bwebkbc|bwebkbm|bwebkci|bwebkec|bwebkso|bwebkem) continue;;
    bwebms|bwebmsbc|bwebmsbm|bwebmsci|bwebmsec|bwebmsso|bwebmsem) continue;;
    bweppatv|bweppgt|bwepppt) continue;;
    txnote) continue;;
  esac
  if $FC $FLAGS "$f" -o "$OBJ/$b.o" 2>/tmp/claude-1000/tt_wsbwe_${b}.err; then nbw=$((nbw+1))
  else echo "WSBWE COMPILE FAIL: $b"; head -3 /tmp/claude-1000/tt_wsbwe_${b}.err; fi
done
...
if [ -n "$OVR" ] && [ -d "$OVR" ]; then
  for f in "$OVR"/*.f; do
    [ -e "$f" ] || continue
    b=$(basename "$f" .f)
    if $FC $FLAGS "$f" -o "$OBJ/$b.o" 2>/tmp/claude-1000/tt_wsbwe_ovr_${b}.err; then echo "OVERRIDE $b"
    else echo "OVERRIDE FAIL: $b"; head -5 /tmp/claude-1000/tt_wsbwe_ovr_${b}.err; fi
  done
fi
echo "buildDir: compiled_ok=$nok fell_back=$nfb ; wsbwe: compiled=$nbw"
COBJ=""; for c in "$SRC"/*.c; do [ -e "$c" ] || continue; COBJ="$COBJ ${c%.c}.o"; done
cd "$OBJ" || exit 3
if $FC -o "$OUT" *.o $COBJ "$SHIM" -lpthread -ldl 2>/tmp/claude-1000/tt_wsbwe_link.err; then
  echo "LINK_OK $OUT"; ls -la "$OUT"
else
  echo "LINK_FAILED:"; grep -i 'undefined\|multiple def' /tmp/claude-1000/tt_wsbwe_link.err | sort -u | head -30
fi
