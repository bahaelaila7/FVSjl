#!/bin/bash
# FVSbm_wsbwe relink — EM object set with base/exbudl.f no-op stub replaced by
...
set -u
SRC=/workspace/ForestVegetationSimulator/bin/FVSbm_buildDir
BWE=/workspace/ForestVegetationSimulator/wsbwe
WORK=/workspace/.bmwork/wsbwe
OBJ=$WORK/emobj
SHIM=/workspace/.crwork/isoc23_shim.o
OVR="${1:-}"
OUT="${2:-$WORK/FVSbm_wsbwe}"
FC=gfortran-16
...
for f in *.f; do
  b=$(basename "$f" .f)
  case "$b" in
    bwebk|bwebkbc|bwebkem|bwebkci|bwebkec|bwebkso|bwebktt) continue;;
    bwebms|bwebmsbc|bwebmsem|bwebmsci|bwebmsec|bwebmsso|bwebmstt) continue;;
...
cd "$OBJ" || exit 3
if $FC -o "$OUT" *.o $COBJ "$SHIM" -lpthread -ldl 2>/tmp/claude-1000/bm_wsbwe_link.err; then
  echo "LINK_OK $OUT"; ls -la "$OUT"
else
  echo "LINK_FAILED:"; grep -i 'undefined\|multiple def' /tmp/claude-1000/bm_wsbwe_link.err | sort -u | head -30
fi
