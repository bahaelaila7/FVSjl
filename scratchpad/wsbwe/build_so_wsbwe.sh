#!/bin/bash
# FVSso_wsbwe relink — EM object set with base/exbudl.f no-op stub replaced by
...
set -u
SRC=/workspace/ForestVegetationSimulator/bin/FVSso_buildDir
BWE=/workspace/ForestVegetationSimulator/wsbwe
WORK=/workspace/.sowork/wsbwe
OBJ=$WORK/emobj
SHIM=/workspace/.crwork/isoc23_shim.o
OVR="${1:-}"
OUT="${2:-$WORK/FVSso_wsbwe}"
FC=gfortran-16
...
for f in *.f; do
  b=$(basename "$f" .f)
  case "$b" in
    bwebk|bwebkbc|bwebkbm|bwebkci|bwebkec|bwebkem|bwebktt) continue;;
    bwebms|bwebmsbc|bwebmsbm|bwebmsci|bwebmsec|bwebmsem|bwebmstt) continue;;
...
cd "$OBJ" || exit 3
if $FC -o "$OUT" *.o $COBJ "$SHIM" -lpthread -ldl 2>/tmp/claude-1000/so_wsbwe_link.err; then
  echo "LINK_OK $OUT"; ls -la "$OUT"
else
  echo "LINK_FAILED:"; grep -i 'undefined\|multiple def' /tmp/claude-1000/so_wsbwe_link.err | sort -u | head -30
fi
