...
C
      ...
      OPEN(79,FILE='bwepdm_dump.txt',STATUS='UNKNOWN',
     &     POSITION='APPEND')
C
C
C     ********************** EXECUTION BEGINS **************************
...
C
     ...
     &            (B6(IHOST)*KTK)+(B7(IHOST)*MFT*MFM)) )
            ELSE
               PR = 0.0
            ENDIF
C=== INSTRUMENT: deterministic mortality kernel dump (pre-RNG) ===
            WRITE(79,7901) ICYC,I,ISPI,IHOST,ISZI,
     &        TRANSFER(ELEV,IHX79),
     &        TRANSFER(PNTBA(ITRE(I)),IHX79),
     &        TRANSFER(PNTHBA(ITRE(I)),IHX79),
     &        TRANSFER(MFT,IHX79),TRANSFER(MFM,IHX79),
     &        TRANSFER(KTK,IHX79),
     &        TRANSFER(PEDDS(IHOST,ISZI),IHX79),
     &        TRANSFER(PEHTG(IHOST,ISZI),IHX79),
     &        TRANSFER(BWMXCD(IHOST,ISZI),IHX79),
     &        TRANSFER(PR,IHX79)
 7901       FORMAT('PDM',5(1X,I5),10(1X,Z8.8))
